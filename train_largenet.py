...
=========================================
Basis:    Multi-Coin (BTC+ETH+BNB) – Phase 1 Gewinner
Änderung: hidden_dim 128 -> 256  (doppelte GRU-Kapazität)
Bonus:    Multi-TF Features wieder aktiv (Phase 2 scheiterte wegen zu kleinem Netz)
...
HIDDEN = 256  # <── der einzige Unterschied zu Phase 1


# ── Multi-TF Features (gleiche Logik wie Phase 2) ─────────────────────────────
        ...
        tf_df = pd.DataFrame(
            {
                ...
                f"volatility_{label}": close.pct_change().rolling(20).std() * np.sqrt(252),
            },
            index=r.index,
...
def load_and_prepare():
    ...
    for sym in SYMBOLS:
        ...
        combined = pd.concat([std_f.loc[ci], train_p.loc[ci, mtf_cols].fillna(0)], axis=1)

        prices_all.append(train_p[ohlcv].loc[ci])
        feats_all.append(combined)
        logger.info(f"  {sym}: {len(combined)} Schritte, {len(combined.columns)} Features")

    price = pd.concat(prices_all, ignore_index=True)
    feats = pd.concat(feats_all, ignore_index=True).fillna(0)
    logger.info(f"Gesamt: {len(price)} Schritte, {len(feats.columns)} Features")
    return price, feats
...
def train(price, feats):
    ...
    logger.info(f"Netz: hidden_dim={HIDDEN}  |  state_dim={sd}  |  actions={na}")

    trader_cfg = PPOConfig(
        state_dim=sd,
        n_actions=na,
        hidden_dim=HIDDEN,  # ← 256 statt 128
...
def compare(baseline, largenet):
    logger.info("\n" + "═" * 62)
    logger.info("  VERGLEICH: Multi-Coin (hidden=128)  vs.  Large Net (hidden=256)")
    logger.info("═" * 62)
    logger.info(f"  {'Metrik':<22} {'h=128':>12} {'h=256':>12} {'Δ':>8}")
    logger.info("─" * 62)
    checks = [
    ...
    logger.info(f"  FAZIT: Large Net ist {v} ({better}/4 Metriken verbessert)")
    logger.info("═" * 62)
    return v
...
if __name__ == "__main__":
    ...
    logger.add(
        f"logs/training/largenet_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.log", level="DEBUG"
    )

    Path(SAVE_DIR).mkdir(parents=True, exist_ok=True)

    logger.info("════ PHASE 3: LARGE NETWORK TRAINING ════")
    logger.info(f"hidden_dim: 128 → {HIDDEN}  |  Mit Multi-TF Features")

    price, feats = load_and_prepare()
    metrics = train(price, feats)

    with open(f"{SAVE_DIR}/comparison.json", "w") as f:
        json.dump({"baseline_multicoin": BASELINE_MULTICOIN, "largenet": metrics}, f, indent=2)

    verdict = compare(BASELINE_MULTICOIN, metrics)
    logger.info(f"Modell: {SAVE_DIR}/")
    logger.info(
        f"Nächste Phase: {'Phase 4 (Curriculum Learning)' if verdict != 'SCHLECHTER' else 'hidden_dim=128 bleibt Basis'}"
    )
