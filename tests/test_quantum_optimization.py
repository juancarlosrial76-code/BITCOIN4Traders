...
class TestQuantumAnnealingOptimizer:
    ...
    def test_hamiltonian_creation(self, sample_returns):
        ...
        expected_returns = sample_returns.mean().values * 252
        covariance = sample_returns.cov().values * 252
        ...
        assert H.shape == (3, 3)
        assert np.all(np.diag(H) != 0)  # Local fields present
...
class TestQAOASimulator:
    ...
    def test_initialization(self):
        ...
        assert len(qaoa.gamma) == 3
        assert len(qaoa.beta) == 3
...
class TestQuantumInspiredEvolution:
    ...
    def test_quantum_crossover(self):
        ...
        child = evo._quantum_crossover(parent1, parent2)

        assert len(child.amplitudes) == 5
        assert np.abs(np.linalg.norm(child.amplitudes) - 1.0) < 1e-6
    ...
    def test_quantum_mutation(self):
        ...
        mutated = evo._quantum_mutation(state)

        assert len(mutated.amplitudes) == 5
        assert np.abs(np.linalg.norm(mutated.amplitudes) - 1.0) < 1e-6
...
class TestEdgeCases:
    ...
    def test_empty_returns(self):
        ...
        with pytest.raises(Exception):
            optimizer.optimize_portfolio(empty_returns)
