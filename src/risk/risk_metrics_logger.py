...
class RiskMetricsLogger:
    ...
    def calculate_sharpe_ratio(self, annualization: float = 252.0) -> float:
        ...
        # Sharpe = (mean_return - rfr) / std_return, then annualize: × √(annualization)
        sharpe = (mean_return - self.risk_free_rate / annualization) / std_return
        sharpe = sharpe * np.sqrt(annualization)  # Scale from per-step to per-year
    ...
    def calculate_sortino_ratio(self, annualization: float = 252.0) -> float:
        ...
        # Sortino uses downside deviation (only negative returns) instead of total std
        sortino = (mean_return - self.risk_free_rate / annualization) / downside_std
        sortino = sortino * np.sqrt(annualization)  # Annualize same as Sharpe
    ...
    def calculate_calmar_ratio(self) -> float:
        ...
        # Annualized return: compound the total return to a yearly rate
        total_return = (
        ...
        annualized_return = (1 + total_return) ** (
            252 / n_periods
        ) - 1  # Geometric annualization

        # Max drawdown from peak over full history
