...
class HandlerRegistry:
    ...
    async def dispatch(self, stream: str, data: dict) -> None:
        for h in self._handlers.get(stream, []):
            try:
                await h(data)
            except Exception as exc:
                logger.error("Handler for '%s' raised: %s", stream, exc, exc_info=True)
...
class BinanceWSConnector:
    ...
    async def _do_connect(self) -> None:
        ...
        try:
            ...
            self._reconnect_attempts = 0
            logger.info("WebSocket connected (%s streams active)", len(self._subscriptions))
    ...
    async def _handle_message(self, raw: str) -> None:
        ...
        if stream:
            await self._registry.dispatch(stream, payload)
        elif "e" in payload:
            # User data stream events have no 'stream' wrapper
            await self._registry.dispatch(payload["e"], payload)
    ...
    async def _start_user_stream(self) -> None:
        try:
            ...
            logger.info("User data stream started (listenKey=...%s)", self._listen_key[-8:])
        except Exception as exc:
    ...
    async def _refresh_listen_key(self) -> None:
        if self._listen_key:
            await self._rest_put("/api/v3/userDataStream", {"listenKey": self._listen_key})
    ...
    async def _send_request(self, method: str, params: List[str]) -> None:
        ...
        self._req_id += 1
        payload = {"method": method, "params": params, "id": self._req_id}
