...
class DataProcessorConfig:
    ...
    def __post_init__(self):
        if self.tech_indicator_list is None:
            self.tech_indicator_list = ['macd', 'rsi', 'cci', 'adx', 'bbs']"
    ...
    def add_technical_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for ticker in self._get_unique_tickers(df):
            ...
            if 'adx' in self.config.tech_indicator_list:
                adx = ADXIndicator(high=df[high_col], low=df[low_col], close=df[close_col])
    ...
    def download_data(
        ...
        **kwargs
        ...
        for i, ticker in enumerate(tickers):
            ...
            df = pd.DataFrame(
                ohlcv,
                columns=['timestamp', f'open_{i}', f'high_{i}', f'low_{i}', f'close_{i}', f'volume_{i}']
            )
