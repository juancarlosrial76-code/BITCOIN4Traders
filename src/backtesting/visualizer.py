...
class BacktestVisualizer:
    ...
    def _plot_equity_curve(self, ax: plt.Axes, equity: pd.Series):
        ...
        if len(returns) > 0:
            ...
            ax.plot(equity.index, benchmark, linewidth=1, 
                   alpha=0.5, linestyle='--', label='Mean Return')
    ...
    def _plot_drawdown(self, ax: plt.Axes, equity: pd.Series):
        ...
        ax.axhline(max_dd * 100, color='red', linestyle='--', 
                  alpha=0.7, linewidth=1)
        ax.text(drawdown.index[len(drawdown)//2], max_dd * 100, 
    ...
    def _plot_rolling_sharpe(self, ax: plt.Axes, equity: pd.Series, window: int = 60):
        ...
        ax.set_title(f'Rolling Sharpe Ratio ({window}d)', fontsize=12, fontweight='bold')
        ax.set_xlabel('Date')
    ...
    def plot_walk_forward_results(
        ...
        ax.axvline(np.mean(test_returns), color='green', linestyle='--', 
                  linewidth=2, label=f'Mean: {np.mean(test_returns):.2f}%')
