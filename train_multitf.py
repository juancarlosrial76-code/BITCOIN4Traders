...
==========================================
Builds on the Multi-Coin result (Phase 1).
Adds 4h and 1d signals as additional features,
computed from the existing 1h data via resampling.

New features (6 additional, no leakage):
...
def add_multitf_features(price_1h: pd.DataFrame) -> pd.DataFrame:
    """
    Resamples 1h data to 4h and 1d, computes Trend/RSI/MACD,
    merges back to 1h (forward-fill → no leakage).
    """
    df = price_1h.copy()
    ...
    for label, rule in [("4h", "4h"), ("1d", "1d")]:
        ...
        # Reindex to 1h, forward-fill (past values only → no leakage)
...
def load_and_prepare(symbols):
    ...
    for sym in symbols:
        logger.info(f"Processing {sym} with Multi-TF Features...")
        price_raw = loader.download_and_cache(sym, TIMEFRAME, START)

        # Add Multi-TF Features BEFORE the split (resampling needs full history)
        ...
        # Fit FeatureEngine on the OHLCV part (standard features)
        eng = FeatureEngine(feat_cfg)
        ohlcv_cols = ["open", "high", "low", "close", "volume"]
        train_ohlcv = train_p[ohlcv_cols]
        train_std = eng.fit_transform(train_ohlcv)

        # Append Multi-TF columns (already forward-filled, no leakage)
    ...
    logger.info(
        f"Total dataset: {len(combined_price)} steps, "
...
def train(price, feats, save_dir):
    ...
    logger.info(f"Observation Space: {state_dim} features (incl. Multi-TF)")
...
def compare(baseline, multitf):
    logger.info("\n" + "═" * 60)
    logger.info("  COMPARISON: Multi-Coin  vs.  Multi-Coin + Multi-Timeframe")
    logger.info("═" * 60)
    logger.info(f"  {'Metric':<22} {'Multi-Coin':>12} {'+ Multi-TF':>12} {'Δ':>8}")
    logger.info("─" * 60)
    ...
    logger.info("═" * 60)
    verdict = "BETTER" if better >= 3 else ("NEUTRAL" if better >= 2 else "WORSE")
    logger.info(f"  RESULT: Multi-TF is {verdict} ({better}/4 metrics improved)")
    logger.info("═" * 60)
    return verdict
...
if __name__ == "__main__":
    ...
    logger.add(
        f"logs/training/multitf_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.log",
        level="DEBUG",
    )
    ...
    with open(f"{SAVE_DIR}/comparison.json", "w") as f:
        json.dump(
            {"baseline_multicoin": BASELINE_MULTICOIN, "multitf": metrics}, f, indent=2
        )

    verdict = compare(BASELINE_MULTICOIN, metrics)
    logger.info(f"\nModel saved: {SAVE_DIR}/")
    logger.info(
        f"Next phase: {'Phase 3 (Larger Network)' if verdict != 'WORSE' else 'Stay with Multi-Coin'}"
    )
